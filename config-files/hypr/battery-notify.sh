...
while true; do
	...
	if [ "$BATTERY_LEVEL" -gt 99 ] && [ "$BATTERY_DISCHARGING" -eq 0 ]; then
		notify-send "󱟢 Battery Charged" "Battery is fully charged." -u low
		# If the battery is low and is not charging (and has not shown notification yet)
	elif [ "$BATTERY_LEVEL" -le $WARNING_LEVEL ] && [ "$BATTERY_DISCHARGING" -eq 1 ]; then
		notify-send "󰁻 Low Battery" "${BATTERY_LEVEL}% of battery remaining." -u critical
		...
		notify-send "󰂃 Battery Critical" "The computer will shutdown soon." -u critical
	fi
	sleep 5
done
