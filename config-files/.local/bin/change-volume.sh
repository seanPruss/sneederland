...
get_volume_bar() {
	...
	local bar=""
	local total_bars=10
	local filled_bars=$((volume * total_bars / 100))
...
notify_user() {
	local volume=$(get_volume)
	local bar=$(get_volume_bar)
	notify-send -h string:x-canonical-private-synchronous:sys-notify -u low -i "$(get_icon)" "Volume : $volume % $bar"
}
